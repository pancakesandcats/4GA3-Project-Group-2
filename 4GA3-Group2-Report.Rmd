...
ggplot() +
  geom_sf(data = tx_tracts,
          fill = "lightgrey") +
  theme_bw() +
  labs(title = "Map of Texas, United States Tracts")
```
...
ggplot(df_race, aes(fill = total_pop)) + 
  ...
  labs(title = "Map of Total Texas Population Distribution")
```
...
ggplot(df_race, aes(fill = white_p)) + 
  ...
  theme_bw() +
  scale_fill_viridis_c(option = "turbo") +
  labs(title = "Proportion of White Population in Texas Tracts")
```
...
ggplot(df_race, aes(fill = black_p)) + 
  ...
  theme_bw() +
  scale_fill_viridis_c(option = "turbo") +
  labs(title = "Proportion of Black Population in Texas Tracts")
```
...
ggplot(df_race, aes(fill = asian_p)) + 
  ...
  theme_bw() +
  scale_fill_viridis_c(option = "turbo") +
  labs(title = "Proportion of Asian Population in Texas Tracts")
```
...
ggplot(df_race, aes(fill = american_indian_p)) + 
  ...
  theme_bw() +
  scale_fill_viridis_c(option = "turbo") +
  labs(title = "Proportion of American Indian Population in Texas Tracts")
```
...
ggplot(df_race, aes(fill = pacific_islander_p)) + 
  ...
  theme_bw() +
  scale_fill_viridis_c(option = "turbo") +
  labs(title = "Proportion of Pacific Islander Population in Texas Tracts")
```
...
ggplot(df_race, aes(fill = food_stamp_p)) + 
  ...
  theme_bw() +
  scale_fill_viridis_c(option = "turbo") +
  labs(title = "Proportion of Food Stamp Usage in Texas Tracts")
```
...
# Conclusion
  ...
  Throughout the use of different inferential and spatial statistical methods, patterns of food access inequalities within counties of Texas were revealed relationships between race, income, location, and SNAP usage. There are clear patterns of spatial autocorrelation, which suggest that high food stamp usage is prevalent in the southern areas of Texas, shown by the clustering pattern portrayed in the map for the spatial moving average and the Gi* plot. This clustering supports the conclusion that the use of food stamps in Texas counties is not random. Additionally, when comparing SMA maps between food stamp usage and racial groups, some co-location patterns emerge. For example, Black population SMA hotspots often overlap with areas of high food stamp usage, especially in eastern Texas. This spatial relationship suggests that Black-majority counties may be more vulnerable to food insecurity. Meanwhile, the white population is concentrated in central and northwestern regions, which tend to show lower food stamp usage, highlighting a potential spatial divide between majority-white counties and areas facing economic vulnerability.
  
  The clustering patterns may suggest that regions with high food stamp usage also have high concentrations of specific racial groups, which can lead to structural and system inequalities within the community. Results from the Gi* hotspot analysis further confirmed this. Statistically significant Gi* hotspots for food stamp usage were observed in Texas's southern and southwestern border regions, most notably along the Rio Grande Valley. Many of these hotspots overlap with areas of lower white population density and higher concentrations of racially diverse communities, particularly those identified as Black, "Other," and American Indian. The Black population displayed strong hotspots in the east, including urban centers, indicating persistent historical inequities and segregation. The Asian population showed a unique clustering pattern, concentrated around urban centers like Houston and Dallas, with comparatively lower food stamp usage. This pattern may reflect more stable socioeconomic conditions among some Asian communities in urban areas. In contrast, the Pacific Islander and "Other" groups had scattered or localized clusters that also intersected with food insecurity zones, suggesting more complex or underrepresented vulnerabilities.

  These spatial trends are further supported by the regression analysis, which confirms that race remains a statistically significant predictor of food stamp usage even after controlling for income, unemployment, and household composition. Tracts with higher shares of Black and "Other" populations were significantly more likely to report increased food stamp reliance, while Asian-majority tracts were associated with lower usage relative to the White reference group.

  Together, these findings support the study's original research questions and reinforce key themes from the background literature. First, there is substantial variation in food stamp usage across counties, driven in part by racial composition. Second, statistically significant spatial clusters exist for food insecurity and racial demographics, and their co-location provides strong visual and analytical evidence of structural disparities. Lastly, spatial analysis confirms that food insecurity in Texas is patterned by more than just income or geography, and that race plays a significant role even after accounting for other factors, as further supported by regression analysis in later sections.

## Recommendations

  To build on the findings of this study, further research should investigate how intersecting factors such as race, geographic location, and socioeconomic status compound food insecurity risk. While this study relied on proxies such as food stamp usage and racial composition, future work could incorporate finer-grained data (e.g., food access infrastructure, program enrollment barriers, transportation availability) to capture better the lived experience of food insecurity in Texas communities. Additional spatial methods, including spatial lag or geographically weighted regression at finer scales, would further clarify causal pathways and regional variability.
  
  From a policy standpoint, spatially targeted food access interventions should prioritize the hotspot regions identified in this study. Particular areas of concern are southern and eastern counties where food stamp usage and racialized disadvantage are highly concentrated. These are not just areas of economic deprivation, but zones where structural inequalities have persisted. Programs like SNAP should also be evaluated on factors such as spatial accessibility, diet quality, and equitable outcomes across racial groups. Expanding mobile grocery services, incentivizing supermarkets in underserved areas, and increasing culturally relevant outreach could address access barriers specific to Black, Indigenous, and racially diverse populations. Policy must move beyond income-based thresholds to recognize the spatial and racial dimensions of food insecurity that our findings reveal.
